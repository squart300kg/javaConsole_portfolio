...
public class 스쿼트 extends 삼대운동 {
	...
	int 중량_반복수_세트수_설정(int one_RM) {
		...
		System.out.printf("[ 1. 중량 설정 - 최소 90 ] : ");
	...
	void 스쿼트수행하기(int 경고메시지무시여부) {
		...
		for(int 세트수반복변수 = 1 ; 세트수반복변수 <= 세트수 ; 세트수반복변수 ++) {//세트를 반복한다.
			for(int 횟수반복변수 = 1 ; 횟수반복변수 <= 반복수 ; 횟수반복변수 ++) {//1회씩 운동을 반복한다.
				...
				System.out.println("[ 스쿼트를 진행해 보자 ]");
				System.out.println("┌────────────────────────────────────────────────┐");
				...
				if(운동수행여부 == 1) {//운동을 수행한다면!
					...
					//스쿼트를 수행함에 따라 중량설정값을 디폴트 비율로 하여 횟수와 셋트수를 반복할 때마다 HP가 떨어진다.
					홍길동.HP -= 중량/10;//스쿼트 1회를 반복할 때마다 중량의 1/10배씩 HP가 감소한다.
					System.out.println("┌────────────────────────────────────────────────┐");
					System.out.println("│              스쿼트 - "+세트수반복변수+"세트/"+세트수+"세트,  "+횟수반복변수+"회/"+반복수+"회      	");
					System.out.println("│                     앉았다 일어나기 성공!!               ");
					System.out.println("│                   동이의 현재 HP : "+홍길동.HP+"              ");
					System.out.println("└────────────────────────────────────────────────┘");
					//if(홍길동.아버지옷착용여부)System.out.println("[ 주변사람들의 관심 : 어머..사람 봐... 이상한 옷 입고 운동하고 있어..........(수근수근...) ]");
					홍길동.스쿼트1RM += 중량/50 * (반복수/2 * 세트수/2);//중량의 1퍼센트 비율로 반복수 세트수만큼 오른다.
					홍길동.삼대중량 += 중량/50 * (반복수/2 + 세트수/2);
					if(홍길동.HP < 0) {
						System.out.println("====================================================");
						System.out.println("|                                                  |");
						System.out.println("|    길동이는 욕심에 눈이 멀어 허리가 접질려져 버렸다고 한다 끝          |");
						System.out.println("|                                                  |");
						System.out.println("====================================================");
					}
			...
			for(int 초 = 1 ; 초 <= 쉬는시간 ; 초 ++) {//세트사이 길동이의 휴식시간이다.
				try {
			...
			if(세트수반복변수 == 세트수 && 횟수반복변수 == 0) {//스쿼트의 반복수와 세트수를 모두 채웠을 때
				System.out.println("[ 스쿼트는 성공적으로 클리어 했다!! ]");
				홍길동.길동이의신체상황();
			}
		...
		//System.out.println("인터럽트상태 : " + 아빠옷.isInterrupted());
		if(홍길동.아버지옷착용여부) {
			아빠옷.interrupt();			
		}
		//System.out.println("인터럽트상태 : " + 아빠옷.isInterrupted());
