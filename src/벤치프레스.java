...
public class 벤치프레스 extends 삼대운동 {
	홍길동 홍길동;
	...
	int 중량_반복수_세트수_설정(int one_RM) {
		...
		System.out.printf("[ 1. 중량 설정 - 최소 80] : ");
	...
	void 벤치프레스수행하기(int 경고메시지무시여부){
		...
		for(int 세트수반복변수 = 1 ; 세트수반복변수 <= 세트수 ; 세트수반복변수 ++) {//세트를 반복한다.
			for(int 횟수반복변수 = 1 ; 횟수반복변수 <= 반복수 ; 횟수반복변수 ++) {//1회씩 운동을 반복한다.
				...
				System.out.println("[ 벤치프레스를 진행해 보자 ]");
				System.out.println("===================================================");
				...
				if(운동수행여부 == 1) {//운동을 수행한다면!
					...
					//벤치프레스를 수행함에 따라 중량설정값을 디폴트 비율로 하여 횟수와 셋트수를 반복할 때마다 HP가 떨어진다.
					홍길동.HP -= 중량/10;//벤치프레스 1회를 반복할 때마다 중량의 1/10배씩 HP가 감소한다.
					System.out.println("┌────────────────────────────────────────────────┐");
					System.out.println("                          벤치프레스 - "+세트수반복변수+"세트/"+세트수+"세트,  "+횟수반복변수+"회/"+반복수+"회              ");
					System.out.println("                                  길동이의 현재 HP : "+홍길동.HP+"              ");
					System.out.println("                                  밀어 올리기 성공!!                 ");
					System.out.println("└────────────────────────────────────────────────┘");					
					홍길동.벤치프레스1RM += 중량/40 * (반복수/2 * 세트수/2);//중량의 1퍼센트 비율로 반복수 세트수만큼 오른다.
					홍길동.삼대중량 += 중량/40 * (반복수/2 + 세트수/2);
			...
			if(세트수반복변수 == 세트수 && 횟수반복변수 == 0) {//벤치프레스의 반복수와 세트수를 모두 채웠을 때
				System.out.println("[ 벤치프레스를 성공적으로 클리어 했다!! ]");
				홍길동.길동이의신체상황();
			}
